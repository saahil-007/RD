...
import { Upload, X, Download, Sparkles, Eye } from 'lucide-react';
...
const ImageUpload: React.FC<ImageUploadProps> = ({ 
  ...
  const analyzeFile = (fileId: string) => {
    ...
    formData.append('image', fileToAnalyze.file);
                ...
                lines.forEach(line => {
                    if (line.startsWith('data:')) {
                        try {
                            ...
                            if (data.report_part) {
                                console.log('📊 Analysis Progress:', data.report_part);
                                setFiles(prev => prev.map(f => 
                                    f.id === fileId 
                                    ? { ...f, analysis: { ...f.analysis, ...data.report_part } }
                                    : f
                                ));
                            } else if (data.report) {
                                console.log('✅ Final Analysis Report:', data.report);
                                setFiles(prev => prev.map(f => 
                                    f.id === fileId 
                                    ? { ...f, analysisStatus: 'complete' as const, analysis: { ...f.analysis, ...data.report }, progress: 100 }
                                    : f
                                ));
                                if (onAnalysis) {
  ...
  const renderAnalysis = (file: UploadedFile) => {
    if (!file.analysis) return null;

    return (
      <div className="space-y-4 mt-4">
        <div className="flex justify-between items-center">
          <h6 className="font-semibold text-sm">Analysis Results</h6>
          <div className="flex space-x-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => setShowRawData(showRawData === file.id ? null : file.id)}
            >
              <Eye className="w-4 h-4 mr-2" />
              {showRawData === file.id ? 'Hide' : 'Show'} Raw Data
            </Button>
            <Button
              variant="outline"
              size="sm"
          ...
          {showRawData === file.id && (
            ...
            >
              <Card className="p-4 bg-muted/50 border-2 border-dashed">
                ...
                <pre className="whitespace-pre-wrap break-all text-xs bg-background p-3 rounded border overflow-auto max-h-96">
            ...
            {files.map((file) => (
              <Card key={file.id} className="overflow-hidden group hover:shadow-heritage transition-all duration-300">
                <div className="relative">
                  <img
                    src={file.publicUrl || file.preview}
                    alt={file.name}
                ...
                <CardContent className="p-4">
                  ...
                  {file.analysisStatus === 'analyzing' && (
                    <AnalysisLoading 
                      ...
                      currentStep={file.progressDescription}
                      estimatedTime={file.estimatedTime}
                    />
                  )}
                  
                  {file.analysis && file.analysisStatus === 'complete' && renderAnalysis(file)}
                </CardContent>
              </Card>
            ))}
          </div>
